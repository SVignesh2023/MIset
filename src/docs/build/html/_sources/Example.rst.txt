...
**NOTE** - It is **generally recommended** to apply binning to both continuous and discrete variables before using this feature selection technique, as this was the approach taken by its authors.
...
Now that feature selection is done, we use :py:meth:`MIset.feature_ranking` method to get the top 3 relevant features found by the feature selection algorithm from our entire dataset.

:py:meth:`MIset.feature_ranking` gives a dictionary where the dictionary key is the iteration number and the dictionary value is the most important feature found in that iteration.
